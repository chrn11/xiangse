...
| 8.7 | WebView | `10d89e3`；`load(URLRequest)` 章节 http(s)；didFinish/debug 非 about:blank；dump 针+萧炎。见 `phase87-webview-progress.md` | ✅ PASS |
| 8.8 | 登录 | `legado://login` → UIAlert「书源登录」真机 PASS | ✅ 深链；全链 Cookie 待加强（可开） |
...
真机报告：`fixtures/_devkit/phase8_parity/report.json`（login/explore 深链）；8.5：`fixtures/_devkit/phase85_cache/report.json`；8.6：`fixtures/_devkit/phase86_purify/report.json`；8.7：`fixtures/_devkit/phase87_webview/report.json`（PASS 真导航）。

...
/// 假设 F：同步补链 queryCpFile→division→onDivisionTextFinish（禁 setPageModel / UI fallback）
void LBLoadCurCpBridgeKickDivisionChain(id readerVC, id container, NSDictionary *payload) {
    ...
    if ([c isKindOfClass:[NSString class]]) body = (NSString *)c;
    if (body.length == 0) {
        LBAppendOpenReaderTrace(@"kickDivision_sync skip noBody");
        return;
    }
    ...
    if (!host) return;
    LBAppendOpenReaderTrace([NSString stringWithFormat:
                             @"kickDivision_sync enter host=%@ reader=%@ len=%lu idx=%ld",
    ...
    if ([host respondsToSelector:qfSel]) {
        ...
        for (id arg0 in @[ [NSNull null], body, config ]) {
            id first = (arg0 == (id)[NSNull null]) ? nil : arg0;
            @try {
                ((void (*)(id, SEL, id, id, id))objc_msgSend)(
                    host, qfSel, first, config, userInfo);
                LBAppendOpenReaderTrace([NSString stringWithFormat:
                                         @"kickDivision_sync lpNetWorkDelegateQueryFinish OK arg0=%@",
                                         first ? NSStringFromClass([first class]) : @"nil"]);
                return;
            } @catch (NSException *ex) {
                LBAppendOpenReaderTrace([NSString stringWithFormat:
                                         @"kickDivision_sync lpNetWorkDelegateQueryFinish EX %@",
                                         ex.reason ?: @""]);
            }
        }
    }
    ...
    if (sContentInjectBusy) {
        LBAppendOpenReaderTrace(@"kickDivision_sync skip busy");
        return;
    }
    sContentInjectBusy = YES;
    @try {
        ...
        if (!pageResult) {
            LBAppendOpenReaderTrace(@"kickDivision_sync narrowChain miss divisionText");
            return;
        }
        ...
        if (finishArg && [host respondsToSelector:finish]) {
            @try {
                ...
                LBAppendOpenReaderTrace([NSString stringWithFormat:
                                         @"kickDivision_sync onDivisionTextFinish EX %@",
                                         ex.reason ?: @""]);
            }
        }
        LBAppendOpenReaderTrace([NSString stringWithFormat:
                                 @"kickDivision_sync narrowChain paths=%@",

...
/// 假设 E 窄路径：invoke loadCurCp OK 后补 queryCpFile→division→onDivisionTextFinish（禁 contentInject UI fallback）
void LBLoadCurCpBridgeKickDivisionChain(id readerVC, id container, NSDictionary *payload) {
    if (![payload isKindOfClass:[NSDictionary class]] || payload.count == 0) return;
    NSString *body = nil;
    id c = payload[@"chapterContent"] ?: payload[@"content"];
    if ([c isKindOfClass:[NSString class]]) body = (NSString *)c;
    if (body.length == 0) {
        LBAppendOpenReaderTrace(@"kickDivision skip noBody");
        return;
    }
    ...
    if (!host) return;
    LBAppendOpenReaderTrace([NSString stringWithFormat:
                             @"kickDivision enter host=%@ reader=%@ len=%lu idx=%ld",
                             NSStringFromClass(object_getClass(host)),
                             readerVC ? NSStringFromClass(object_getClass(readerVC)) : @"-",
                             (unsigned long)body.length, (long)cpIndex]);

    // 1) confirmed：lpNetWorkDelegateQueryFinish → divisionResponse（模拟 queryCpFile 回调）
    SEL qfSel = NSSelectorFromString(@"lpNetWorkDelegateQueryFinish:config:userInfo:");
    if ([host respondsToSelector:qfSel]) {
        NSDictionary *config = @{
            @"chapterContent": body,
            @"cpContent": body,
            @"content": body,
        ...
        NSArray *firstArgs = @[ [NSNull null], body, config ];
        for (id firstArg in firstArgs) {
            id arg0 = (firstArg == (id)[NSNull null]) ? nil : firstArg;
            @try {
                ((void (*)(id, SEL, id, id, id))objc_msgSend)(
                    host, qfSel, arg0, config, userInfo);
                LBAppendOpenReaderTrace([NSString stringWithFormat:
                                         @"kickDivision lpNetWorkDelegateQueryFinish OK arg0=%@",
                                         arg0 ? NSStringFromClass([arg0 class]) : @"nil"]);
                return;
            } @catch (NSException *ex) {
                LBAppendOpenReaderTrace([NSString stringWithFormat:
                                         @"kickDivision lpNetWorkDelegateQueryFinish EX %@",
                                         ex.reason ?: @""]);
            }
        }
    }

    // 2) confirmed：onResetContentNotify（无参，走原版 ResetContent/读缓存）
    if (readerVC) {
        SEL resetSel = NSSelectorFromString(@"onResetContentNotify");
        if ([readerVC respondsToSelector:resetSel]) {
            @try {
                ((void (*)(id, SEL))objc_msgSend)(readerVC, resetSel);
                LBAppendOpenReaderTrace(@"kickDivision onResetContentNotify OK");
                return;
            } @catch (NSException *ex) {
                LBAppendOpenReaderTrace([NSString stringWithFormat:
                                         @"kickDivision onResetContentNotify EX %@",
                                         ex.reason ?: @""]);
            }
        }
    }

    // 3) 窄 division 链：PaibanManager#divisionText → TextRPageContainer#divisionResponse → onDivisionTextFinish
    if (sContentInjectBusy) {
        LBAppendOpenReaderTrace(@"kickDivision skip busy");
        return;
    }
    sContentInjectBusy = YES;
    @try {
        NSMutableArray *okPaths = [NSMutableArray array];
        CGSize sz = CGSizeMake(350, 500);
        if ([readerVC isKindOfClass:[UIViewController class]] &&
            ((UIViewController *)readerVC).isViewLoaded) {
            sz = ((UIViewController *)readerVC).view.bounds.size;
        }
        if (sz.width < 10 || sz.height < 10) {
            sz = UIScreen.mainScreen.bounds.size;
            sz.width -= 24;
        ...
        id pageResult = nil;
        Class paibanCls = NSClassFromString(@"PaibanManager");
        id pm = nil;
        if (paibanCls) {
            for (NSString *ss in @[@"sharedInstance", @"shared", @"sharedManager"]) {
                SEL s = NSSelectorFromString(ss);
                if ([paibanCls respondsToSelector:s]) {
                    pm = ((id (*)(id, SEL))objc_msgSend)(paibanCls, s);
                    if (pm) break;
                }
            }
        }
        if (pm) {
            pageResult = LBCallDivisionText(pm, NO, body, title, cpIndex, sz, nil);
            if (pageResult) [okPaths addObject:@"divisionText@PaibanManager"];
        }
        if (!pageResult) {
            LBAppendOpenReaderTrace(@"kickDivision narrowChain miss divisionText");
            return;
        }
        id drPages = LBNormalizePageResultForDivision(pageResult, okPaths, sz) ?: pageResult;
        if (!LBInvokeDivisionResponse(host, drPages, title, cpIndex,
                                      sLastDivisionHeights, okPaths)) {
            for (id altHost in LBCollectDivisionHosts((UIViewController *)readerVC)) {
                if (altHost == host) continue;
                if (LBInvokeDivisionResponse(altHost, drPages, title, cpIndex,
                                             sLastDivisionHeights, okPaths)) {
                    host = altHost;
                    break;
                }
        ...
        id finishArg = LBWrapPageResultForOnDivisionTextFinish(pageResult);
        SEL finish = NSSelectorFromString(@"onDivisionTextFinish:cpIndex:");
        if (finishArg && [host respondsToSelector:finish]) {
            @try {
                ((void (*)(id, SEL, id, NSInteger))objc_msgSend)(
                    host, finish, finishArg, cpIndex);
                [okPaths addObject:[NSString stringWithFormat:@"onDivisionTextFinish@%@",
                                    NSStringFromClass(object_getClass(host))]];
            } @catch (NSException *ex) {
                LBAppendOpenReaderTrace([NSString stringWithFormat:
                                         @"kickDivision onDivisionTextFinish EX %@",
                                         ex.reason ?: @""]);
            }
        }
        LBAppendOpenReaderTrace([NSString stringWithFormat:
                                 @"kickDivision narrowChain paths=%@",

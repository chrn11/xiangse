#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""AJ 验收：禁 bg UIWindowScene.windows；idle 当帧 main drain；KEEP inThread；前台硬断言萧炎。"""
...
OUT = ROOT / "fixtures" / "_accept_aj_probe.json"
SYNC = ROOT / "fixtures" / "_accept_aj_probe_sync.json"
KILL = ROOT / "fixtures" / "_accept_aj_kill_syslog.json"
...
def decide(
    ...
    *,
    ...
    if (
        ...
        return {
            "branch": "aj_success",
            "action": "bg scene 已禁 + main drain + 主线程 QF + 萧炎；AJ 成功",
            "commit": True,
        }
    if has_aj_skip and has_aj_drain and qf_main and pid_stable and not has_fatal:
        return {
            "branch": "aj_main_qf_no_xiaoyan",
            "action": "主线程 QF/drain 通但萧炎否；卡在 QF 后上屏",
            "commit": False,
        }
    if has_aj_skip and has_aj_drain and pid_stable and not has_fatal and not qf_main:
        return {
            "branch": "aj_drain_qf_still_bg",
            "action": "drain 出现但 QF 仍 bg（KEEP inThread）；可对照撤 inThread",
            "commit": False,
        }
    if has_aj_skip and scene_syslog_n == 0 and not has_aj_drain:
        return {
            "branch": "aj_skip_no_drain",
            "action": "bg 枚举已禁且 syslog 少/无，但 main drain 未排空",
            "commit": False,
        }
    ...
    if not pid_stable:
        return {
            "branch": "pid_changed",
            "action": "pid 不稳；对照 syslog UIWindowScene/SIGSEGV",
            "commit": False,
        }
    return {
        "branch": "aj_incomplete",
        "action": "AJ 证据不足（skip/drain/QF）",
        "commit": False,
    }
...
def main() -> int:
    ...
    report: dict = {
        ...
        "ipa": str(ipa),
        "hypothesis": "AJ",
        "role": "AJ-ban-bg-UIWindowScene-and-idle-main-drain",
    ...
    for i in range(50):
        ...
        blob = probe + "\n" + st
        ac = [ln for ln in blob.splitlines() if "hypothesis_AC" in ln or "hypothesis_AJ" in ln]
    ...
    try:
        if reader_ok:
            c.call("open_url", {"url": "legado://debugDump?phase=aj_accept"}, timeout=20)
            time.sleep(1.5)
    ...
    ac_all = [
        ln
        for ln in (probe + "\n" + st).splitlines()
        if "hypothesis_AC" in ln or "hypothesis_AJ" in ln
    ]
    ...
    aj_skip = [ln for ln in ac_all if "aj_bg_keywindow_skip" in ln or "aj_bg_allwindows_skip" in ln]
    aj_drain = [
        ln
        for ln in ac_all
        if "aj_main_drain" in ln or "aj_main_pulse" in ln or "aj_main_drain_rl" in ln
    ]
    ai_drain = [ln for ln in ac_all if "ai_main_drain_slot" in ln]
    main_watch = [ln for ln in ac_all if "ai_main_watch" in ln]
    bg_uikit = [ln for ln in ac_all if "ai_bg_uikit" in ln]
    has_aj_skip = len(aj_skip) > 0
    has_aj_drain = len(aj_drain) > 0 or any("ai_main_drain_slot" in ln for ln in ac_all)
    ...
    decision = decide(
        ...
        has_aj_skip=has_aj_skip,
        has_aj_drain=has_aj_drain,
        qf_main=qf_main,
    ...
    sync = {
        ...
        "aj_last_runtime": ac_runtime[-1] if ac_runtime else (ac_all[-1] if ac_all else ""),
        "aj_runtime": ac_runtime[-80:],
        "aj_bg_skip": aj_skip[-12:],
        "aj_main_drain": aj_drain[-8:],
        "ai_main_drain": ai_drain[-4:],
        ...
        "has_aj_main_drain": any("aj_main_drain" in ln for ln in ac_all),
        "has_aj_main_pulse": any("aj_main_pulse" in ln for ln in ac_all),
        ...
        "has_aj_keep_inThread": any("aj_keep_inThread=1" in ln for ln in ac_all),
    ...
    kill_doc = {
        ...
        "sigsegv": sigsegv[:8],
        "aj_bg_skip": aj_skip[-8:],
        "aj_main_drain": aj_drain[-6:],
    }

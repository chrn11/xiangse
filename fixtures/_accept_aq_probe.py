#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""AQ 验收：撤 AE inThread 注入回原版 QF 路径 + 禁 early-wrap 50ms 递归；探针定位 main 排空/pageStatus/container attach；前台硬断言萧炎。"""
...
OUT = ROOT / "fixtures" / "_accept_aq_probe.json"
SYNC = ROOT / "fixtures" / "_accept_aq_probe_sync.json"
KILL = ROOT / "fixtures" / "_accept_aq_kill_syslog.json"
...
AP_KEYS = ("ap_fault_sym", "ap_postqf_stack", "ap_fault_fpstack", "ap_cf_anchor")
AQ_KEYS = (
    "aq_pageStatus_pre",
    "aq_pageStatus_post",
    "aq_orig_imp_class",
    "aq_container_attach",
    "aq_main_drain_pulse",
    "aq_main_drain_result",
    "aq_qf_path_orig",
    "aq_early_wrap_retry_disabled",
)
    ...
    for base in bases:
        for name in names:
            try:
                ...
                pass
    for tmp in ("/tmp/legado_al_fatal.txt", "/tmp/legado_am_hb.txt", "/tmp/legado_an_fault.txt", "/tmp/legado_ao_fault.txt", "/tmp/legado_ao_lbf.txt", "/tmp/legado_ap_fault_fp.txt"):
        try:
...
def _aq_first(lines: list[str], prefix: str) -> str:
    """提取首个含 prefix 的探针行（用于 AQ 字段汇总）。"""
    for ln in lines:
        if prefix in ln:
            return ln.strip()
    return ""
...
def decide(
    *,
    ...
    sigsegv_n: int,
    xiaoyan_ok: bool,
    aq_main_drained: bool,
    aq_pagestatus_pre: str,
    aq_pagestatus_post: str,
    aq_container_attach: str,
    aq_orig_imp_class: str,
    aq_early_wrap_disabled: bool,
    ao_fault_n: int = 0,
    an_fault_n: int = 0,
) -> dict:
    # 成功：main 排空 + QF 在 main + 萧炎 + pid 稳 + 无 SIGSEGV
    if (
        reader_ok
        and xiaoyan_ok
        and has_qf
        ...
        return {
            ...
            "action": "main 排空 + QF 在 main + 萧炎；AQ 真根因定位完成",
            "commit": True,
        }
    # main 排空但仍未萧炎：定位卡在哪步
    if aq_main_drained and not xiaoyan_ok:
        return {
            "branch": "main_drained_no_xiaoyan",
            "action": f"main 排空但未萧炎；pre={aq_pagestatus_pre} post={aq_pagestatus_post} attach={aq_container_attach}",
            "commit": False,
        }
    # main 未排空：真根因确认
    if not aq_main_drained and has_qf:
        return {
            "branch": "main_blocked",
            "action": f"main 未排空（QF 已派发）；pre={aq_pagestatus_pre} post={aq_pagestatus_post} attach={aq_container_attach} orig={aq_orig_imp_class}",
            "commit": False,
        }
    if not aq_main_drained and not has_qf:
        return {
            "branch": "main_blocked_no_qf",
            "action": f"main 未排空且无 QF；pre={aq_pagestatus_pre} post={aq_pagestatus_post} attach={aq_container_attach}",
            "commit": False,
        }
    if ao_fault_n > 0 or an_fault_n > 0:
        return {
            "branch": "fault_pc",
            "action": "故障 PC 已落；对照 AQ 探针",
            "commit": False,
        }
    if not pid_stable and exit_reason_n == 0 and sigsegv_n == 0:
        return {
            "branch": "pid_only_no_sigsegv",
            "action": "无 SIGSEGV，仅 pid 变；对照 AQ 探针与 syslog",
            "commit": False,
        }
    if not pid_stable:
        return {
            "branch": "pid_changed",
            "action": "pid 不稳；对照 AQ 探针 / syslog exit reason",
            "commit": False,
        }
    return {
        "branch": "aq_incomplete",
        "action": "AQ 证据不足",
        "commit": False,
    }
...
def main() -> int:
    ...
    report: dict = {
        ...
        "ipa": str(ipa),
        "hypothesis": "AQ",
        "role": "AQ-revert-inThread-disable-earlywrap-recursion",
        ...
        "banned": [
            ...
            "inject_callback_inThread",
            "early_wrap_50ms_recursion",
        ],
        "keep": [
            "revert_inThread_AE",
            "disable_earlywrap_recursion",
            "V+W+X+Y+Z",
    ...
    ap_fault_sym = [ln for ln in ac_all if "ap_fault_sym" in ln]
    ap_postqf_stack = [ln for ln in ac_all if "ap_postqf_stack" in ln]
    ap_fpstack = [ln for ln in ac_all if "ap_fault_fpstack" in ln]
    ...
    decision = decide(
        ...
        aq_main_drained=any("aq_main_drain_result drained=1" in ln for ln in ac_all),
        aq_pagestatus_pre=_aq_first(ac_all, "aq_pageStatus_pre val="),
        aq_pagestatus_post=_aq_first(ac_all, "aq_pageStatus_post val="),
        aq_container_attach=_aq_first(ac_all, "aq_container_attach state="),
        aq_orig_imp_class=_aq_first(ac_all, "aq_orig_imp_class cls="),
        aq_early_wrap_disabled=any("aq_early_wrap_retry_disabled" in ln for ln in ac_all),
    ...
    sync = {
        ...
        "ap_fault_sym": ap_fault_sym[-24:],
        "ap_postqf_stack": ap_postqf_stack[-8:],
        "ap_fault_fpstack": ap_fpstack[-8:],
        ...
        "aq_orig_imp_class": _aq_first(ac_all, "aq_orig_imp_class cls="),
        "aq_container_attach": _aq_first(ac_all, "aq_container_attach state="),
        "aq_main_drain_pulse": any("aq_main_drain_pulse fired=1" in ln for ln in ac_all),
        "aq_main_drained": any("aq_main_drain_result drained=1" in ln for ln in ac_all),
        "aq_main_drain_result": _aq_first(ac_all, "aq_main_drain_result drained="),
        "aq_qf_path_orig": _aq_first(ac_all, "aq_qf_path_orig action="),
        "aq_early_wrap_retry_disabled": any("aq_early_wrap_retry_disabled" in ln for ln in ac_all),
        "aq_all_lines": [ln for ln in ac_all if any(k in ln for k in AQ_KEYS)],
    }
    ...
    (ROOT / "fixtures" / "_accept_aq_corpse.json").write_text(
        json.dumps(corpse_doc, ensure_ascii=False, indent=2), encoding="utf-8"
    )
    (ROOT / "analysis" / "reader-forensics" / "aq_fault_summary.json").write_text(

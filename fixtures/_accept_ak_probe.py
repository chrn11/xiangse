#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""AK 验收：bg 禁任何 windows API；idle 后 main PC 阻塞点；KEEP inThread；前台硬断言萧炎。"""
...
OUT = ROOT / "fixtures" / "_accept_ak_probe.json"
SYNC = ROOT / "fixtures" / "_accept_ak_probe_sync.json"
KILL = ROOT / "fixtures" / "_accept_ak_kill_syslog.json"
...
def decide(
    ...
    *,
    ...
    if (
        ...
        return {
            "branch": "ak_success",
            "action": "windows API syslog 归零 + 主线程 QF + 萧炎；AK 成功",
            "commit": True,
        }
    if has_ak_skip and scene_syslog_n == 0 and bg_uikit_n == 0 and pid_stable and not has_fatal:
        return {
            "branch": "ak_syslog_cleared_forensics",
            "action": "syslog/bgWin 归零；保留 main 阻塞取证；功能目标部分达成",
            "commit": True,
        }
    if has_ak_skip and scene_syslog_n > 0:
        return {
            "branch": "ak_skip_syslog_remain",
            "action": "已 skip 但仍有 UIWindowScene syslog；疑非 Bridge 或钩前 caller",
            "commit": False,
        }
    if has_ak_block and not qf_main:
        return {
            "branch": "ak_main_block_found",
            "action": "采到 ak_main_block_*；对照符号决定是否最小修",
            "commit": False,
        }
    ...
    return {
        "branch": "ak_incomplete",
        "action": "AK 证据不足（skip/block/syslog）",
        "commit": False,
    }
...
def main() -> int:
    ...
    report: dict = {
        ...
        "ipa": str(ipa),
        "hypothesis": "AK",
        "role": "AK-ban-all-bg-windows-api-and-main-block-forensics",
    ...
    for i in range(50):
        ...
        ac = [
            ...
            if "hypothesis_AC" in ln or "hypothesis_AK" in ln or "ak_main_block" in ln
        ]
    ...
    try:
        if reader_ok:
            c.call("open_url", {"url": "legado://debugDump?phase=ak_accept"}, timeout=20)
            time.sleep(1.5)
    ...
    ak_block = [ln for ln in ac_all if "ak_main_block_" in ln]
    ak_idle = [ln for ln in ac_all if "ak_main_idle" in ln]
    ...
    decision = decide(
        ...
        has_ak_block=has_ak_block,
        qf_main=qf_main,
        scene_syslog_n=len(scene_syslog),
        bg_uikit_n=len(bg_uikit),
    ...
    sync = {
        ...
        "ak_last_runtime": ac_runtime[-1] if ac_runtime else (ac_all[-1] if ac_all else ""),
        "ak_runtime": ac_runtime[-80:],
        "ak_bg_skip": ak_skip[-16:],
        "ak_main_block": ak_block[-16:],
        "ak_main_idle": ak_idle[-8:],
    ...
    kill_doc = {
        ...
        "sigsegv": sigsegv[:8],
        "ak_bg_skip": ak_skip[-10:],
        "ak_main_block": ak_block[-10:],
    }

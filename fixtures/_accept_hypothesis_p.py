#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""假设 P 验收：curPageVC 就绪后再 invoke；禁 kick；观测 QF/DR/上屏。"""
...
OUT = ROOT / "fixtures" / "_accept_hypothesis_p.json"
EXPECTED_SHA = "22536e5"
IPA_DEFAULT = (
    ROOT
    / "dist-ci-run-29563729256"
...
def extract_p_traces(trace_text: str, state_text: str) -> dict:
    blob = trace_text + "\n" + state_text
    keys = (
        "hypothesis_P",
...
def evaluate(trace_text: str, state_text: str, marker_text: str, dump_text: str, xiaoyan: dict | None) -> dict:
    traces = extract_p_traces(trace_text, state_text)
    ...
    nsarraym_exc = (
        "NSArrayM length" in marker_text
        or "NSArrayM length" in trace_text
        or "NSSingleObjectArrayI" in marker_text
        or "NSSingleObjectArrayI" in trace_text
    )
    has_kick = bool(traces["division_kick_sync_begin"])
    has_p = bool(traces["hypothesis_P"])
    ...
    if nsarraym_exc or has_kick:
        verdict = "FAIL_REVERT_P"
        reason = "仍有 Array length 崩溃或人工 kick 回潮"
    elif render_ok or native_chain:
        verdict = "PASS"
        reason = "无崩溃且原生链/上屏证据"
    elif has_p and not render_ok and pm == 0 and not native_chain:
        verdict = "FAIL_NEED_NEXT"
        reason = "P 已执行但仍无 QF/DR/pm，需下一假设（cache 形状或其它前置）"
    else:
        verdict = "FAIL"
        reason = "未命中验收条件"
    ...
    return {
        ...
        "render_ok": render_ok,
        "p_traces": {k: v[-4:] for k, v in traces.items() if v},
    }
...
def main() -> int:
    ...
    parser.add_argument("--sha", type=str, default=EXPECTED_SHA)
    ...
    time.sleep(6.0)
    try:
        c.call("open_url", {"url": "legado://debugDump?phase=hypothesis_p"})
    ...
    print(json.dumps({k: report[k] for k in report if k not in ("dump_tail", "marker_tail")}, ensure_ascii=False, indent=2))

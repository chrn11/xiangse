#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""AT 验收：postQF 窗禁 CFString 密集采样。

承接 AS：撤 inThread 后仍崩在 post_cb_hb 的 CFString 操作（LBANSymbolStack/LBANSampleMemPath）。
AT 禁用这些采样，验证是否消除 postQF CFString 崩溃 + 萧炎上屏。
"""
...
IPA = ROOT / "dist" / "at_3134c40" / "dist" / "StandarReader-legado-bridge-debug.ipa"
OUT = ROOT / "fixtures" / "_accept_hypothesis_at.json"
SHOT = ROOT / "fixtures" / "_accept_hypothesis_at.png"
...
def evaluate(trace: str, state: str, dump: str, diag: str, ab_probe: str,
    ...
    # AT 核心：postQF 禁 CFString 采样探针命中
    at_sampling_disabled = "at_postqf_cfstring_sampling_disabled" in blob
    ...
    if crash:
        if cfstring_crash:
            v, reason = "FAIL_REVERT_AT", "postQF CFString 崩溃未消除（采样禁用无效）"
        else:
            v, reason = "FAIL_REVERT_AT", f"崩溃(maxDepth={max_depth})"
    elif orig_fixed and lbf_storm_gone and main_drained and xy:
        v, reason = "PASS", "orig invoke + 无崩溃 + main 存活 + 萧炎上屏"
        if at_sampling_disabled:
            reason += " + postQF 采样已禁"
        ...
        if not at_sampling_disabled:
            gaps.append("AT 采样禁用探针缺失")
        if not xy:
            gaps.append("萧炎未上屏")
        v = "FAIL_NEED_NEXT" if (orig_fixed or lbf_storm_gone or at_sampling_disabled) else "FAIL"
        reason = ";".join(gaps)
    ...
    return {
        ...
        "at_sampling_disabled": at_sampling_disabled,
...
def main() -> int:
    ...
    report = {"timestamp": ts(), "ipa": str(IPA), "sha": "3134c40", "steps": []}
    ...
    report["probe_hits"] = {
        ...
        "at_sampling_disabled": [ln for ln in blob.splitlines() if "at_postqf_cfstring_sampling_disabled" in ln][-5:],
        "post_cb_hb": [ln for ln in blob.splitlines() if "post_cb_hb" in ln or "LBAMStartPostCbHeartbeat" in ln][-5:],

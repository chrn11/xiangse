#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""AI 取证验收：定位 bg UIWindowScene caller + invoke 后主线程阻塞点；KEEP inThread。"""
...
OUT = ROOT / "fixtures" / "_accept_ai_probe.json"
SYNC = ROOT / "fixtures" / "_accept_ai_probe_sync.json"
KILL = ROOT / "fixtures" / "_accept_ai_kill_syslog.json"
AI_TXT = ROOT / "fixtures" / "_accept_ai_probe_stack.txt"
...
def decide(
    ...
    qf_main = any("qf_enter" in ln and "main=1" in ln for ln in tags)
    if not reader_ok:
        return {
    ...
    if (has("qf_enter") or has_qf) and (
        has("ai_main_drain_slot") or has("qf_dispatch_main_pulse")
    ) and qf_main and pid_stable:
        return {
            "branch": "qf_on_main_drain_ok",
            "action": "主线程 QF + 主队列可排空；可按萧炎裁定最小修",
            "commit": True,
        }
    if has_bg_caller and has_main_block:
        return {
            "branch": "forensics_both",
            "action": "已定位 bg UIKit caller 与 main 阻塞点；写下一刀",
            "commit": False,
        }
    if has_bg_caller:
        return {
            "branch": "forensics_bg_caller",
            "action": "已定位 bg UIWindowScene caller；main 阻塞未明",
            "commit": False,
        }
    if has_main_block:
        return {
            "branch": "forensics_main_block",
            "action": "已定位 main 阻塞点；bg caller 未明",
            "commit": False,
        }
    if has_fatal or any("fatal_signal" in ln for ln in tags):
        return {
            "branch": "fatal_signal",
            "action": "有 fatal_signal；对照 syslog",
            "commit": False,
        }
    if not pid_stable:
        return {
            "branch": "pid_changed",
            "action": "pid 不稳且取证标签不足；对照 syslog UIWindowScene/SIGSEGV",
            "commit": False,
        }
    return {
        "branch": "forensics_incomplete",
        "action": "未打出 bg caller / main 阻塞点",
        "commit": False,
    }
...
def main() -> int:
    ...
    report: dict = {
        ...
        "ipa": str(ipa),
        "hypothesis": "AI",
        "role": "AI-bg-UIWindowScene-caller-and-main-block",
        "model": "cursor-grok-4.5",
        "mock": MOCK,
        "mcp": MCP,
        "banned": ["bounce", "dontFormat", "mid_chain_launch_app", "dispatch_sync_main_QF"],
        "keep": [
            "inject_inThread",
            "V+W+X+Y+Z",
    ...
    probe = c.read_sandbox_text("legado_ab_probe.txt", max_bytes=300000) or ""
    ai_probe = c.read_sandbox_text("legado_ai_probe.txt", max_bytes=400000) or ""
    st = c.read_sandbox_text("legado_loadcurcp_state.txt", max_bytes=300000) or ""
    dump = c.read_sandbox_text("legado_debug_dump.txt", max_bytes=250000) or ""
    try:
        if reader_ok:
            c.call("open_url", {"url": "legado://debugDump?phase=ai_accept"}, timeout=20)
            time.sleep(1.5)
            dump = c.read_sandbox_text("legado_debug_dump.txt", max_bytes=250000) or dump
    except Exception as exc:
        report["dump_err"] = str(exc)
    try:
        AI_TXT.write_text(ai_probe, encoding="utf-8")
    except Exception:
        pass
    ...
    tags = ac_runtime[-80:] if ac_runtime else ac_all[-80:]
    bg_uikit = [ln for ln in (ac_all + ai_probe.splitlines()) if "ai_bg_uikit" in ln]
    bg_tags = [ln for ln in ac_all if "ai_bg_tag=" in ln]
    main_pc = [ln for ln in (ac_all + ai_probe.splitlines()) if "ai_main_block_pc" in ln]
    main_watch = [ln for ln in ac_all if "ai_main_watch" in ln]
    drain_lines = [ln for ln in ac_all if "ai_main_drain_slot" in ln]
    has_bg_caller = len(bg_uikit) > 0 or len(bg_tags) > 0
    has_main_block = len(main_pc) > 0 or (
        any("ai_main_watch" in ln and "drain=0" in ln for ln in ac_all)
        and not any("ai_main_drain_slot" in ln for ln in ac_all)
    )
    decision = decide(
        tags,
        has_qf=has_qf,
    ...
    first_chapter = bool(
        ...
        and any("qf_enter" in ln and "main=1" in ln for ln in ac_all)
        and any("ai_main_drain_slot" in ln or "qf_dispatch_main_pulse" in ln for ln in ac_all)
        and pid_stable
    )
    ...
    sync = {
        ...
        "ai_last_runtime": ac_runtime[-1] if ac_runtime else (ac_all[-1] if ac_all else ""),
        "ai_runtime": ac_runtime[-80:],
        "ai_bg_uikit": bg_uikit[-12:],
        "ai_bg_tags": bg_tags[-12:],
        "ai_main_block_pc": main_pc[-8:],
        "ai_main_watch_tail": main_watch[-10:],
        "ai_main_drain": drain_lines[-4:],
        "ai_probe_tail": [ln for ln in ai_probe.splitlines() if ln.strip()][-20:],
        ...
        "has_ai_main_drain": any("ai_main_drain_slot" in ln for ln in ac_all),
        "has_bg_hb": len(hb) > 0,
        "bg_hb_n": len(hb),
        "bg_hb_tail": hb[-8:],
        ...
        "has_ai_keep_inThread": any("ai_keep_inThread=1" in ln for ln in ac_all),
        ...
        "has_fatal": has_fatal,
        "has_bg_caller": has_bg_caller,
        "has_main_block_evidence": has_main_block,
    ...
    kill_doc = {
        ...
        "mid_chain_launch_app": False,
        "hits": syslog_hits[:40],
        "ai_bg_uikit": bg_uikit[-8:],
        "ai_main_block_pc": main_pc[-6:],
    }

#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""AN 验收：corpse/故障 PC + 符号化 am_icu_caller + hb mem 路径对照；KEEP AK/AM/inThread；前台硬断言萧炎。"""
...
OUT = ROOT / "fixtures" / "_accept_an_probe.json"
SYNC = ROOT / "fixtures" / "_accept_an_probe_sync.json"
KILL = ROOT / "fixtures" / "_accept_an_kill_syslog.json"
    ...
    for base in bases:
        for name in names:
            try:
                ...
                pass
    for tmp in ("/tmp/legado_al_fatal.txt", "/tmp/legado_am_hb.txt"):
        try:
    ...
    *,
    ...
    if (
        ...
        return {
            ...
            "action": "主线程 QF + 萧炎 + pid 稳；AN 成功可最小修落盘",
            "commit": True,
        }
    if an_fault_n > 0 or an_icu_stack_n > 0 or an_mem_path_n > 0:
        return {
            "branch": "an_fault_icu_mem",
            "action": "故障 PC / ICU 符号化栈 / mem 路径已取证；无最小修则只交取证",
            "commit": False,
        }
    if am_hb_n > 0 and (am_icu_n > 0 or exit_reason_n > 0 or not pid_stable):
        return {
            "branch": "am_hb_icu_or_exit",
            "action": "心跳/exit 仍在；AN 故障 PC 未落则加深；无最小修只交取证",
            "commit": False,
        }
    if has_ak_skip and scene_syslog_n == 0 and not pid_stable:
        return {
            "branch": "ak_keep_still_kill",
            "action": "AK KEEP 仍成立但 pid 变；AN 证据不足则加深",
            "commit": False,
        }
    if not pid_stable:
        return {
            "branch": "pid_changed",
            "action": "pid 不稳；对照 an_fault / syslog exit reason / am_hb 末拍",
            "commit": False,
        }
    return {
        "branch": "an_incomplete",
        "action": "AN 证据不足",
        "commit": False,
    }
...
def main() -> int:
    ...
    report: dict = {
        ...
        "ipa": str(ipa),
        "hypothesis": "AN",
        "role": "AN-corpse-fault-pc-icu-stack-mem",
    ...
    try:
        tmp_an_fault = c.call(
            "run_command",
            {
                ...
                "echo '---'; cat /var/mobile/Containers/Data/Application/*/Documents/legado_an_fault.txt 2>/dev/null | tail -n 40; "
    ...
    try:
        if reader_ok:
            c.call("open_url", {"url": "legado://debugDump?phase=an_accept"}, timeout=20)
            time.sleep(1.5)
    ...
    has_fatal = any(
        "fatal_signal" in ln or "al_fatal_signal" in ln or "an_fault_signal" in ln
        for ln in ac_all
    )
    qf_n = sum(
        1
    ...
    corpse_doc = {
        ...
        "crash_stack": report.get("crash_stack") or [],
        "tmp_an_fault": tmp_an_fault_text[:6000],
    ...
    (ROOT / "fixtures" / "_accept_an_corpse.json").write_text(
        json.dumps(corpse_doc, ensure_ascii=False, indent=2), encoding="utf-8"
    )
    (ROOT / "analysis" / "reader-forensics" / "an_fault_summary.json").write_text(

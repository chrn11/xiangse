#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""假设 G 验收：pageContainer getter 只观察 swizzle（enter/leave 取证）。"""
...
OUT = ROOT / "fixtures" / "_accept_hypothesis_g.json"
...
def resolve_ipa() -> Path:
    if len(sys.argv) > 1:
        ...
        if p.is_file():
            return p
    env = __import__("os").environ.get("HYPOTHESIS_G_IPA")
    if env:
        p = Path(env)
        if p.is_file():
            return p
    for pat in (
        ...
        if hits:
            return hits[0]
    raise FileNotFoundError("未找到 legado-debug IPA，请设置 HYPOTHESIS_G_IPA 或先下载 CI artifact")
...
def parse_enter(line: str) -> dict | None:
    m = re.search(
        r"hypothesis_G enter cat=(\d+) type=(-?\d+) bd8=(0x[0-9a-fA-F]+) a=(\S+)",
        line,
    )
    if not m:
        return None
...
def parse_leave(line: str) -> dict | None:
    m = re.search(r"hypothesis_G leave ret=(\S+) children=(\d+)", line)
    if not m:
        return None
    return {"ret": m.group(1), "children": int(m.group(2))}
...
def main() -> int:
    ...
    g_lines = [ln for ln in blob.splitlines() if "hypothesis_G" in ln]
    f_lines = [ln for ln in blob.splitlines() if "hypothesis_F" in ln]
    c_lines = [ln for ln in blob.splitlines() if "hypothesis_C" in ln]
    report["g_lines"] = g_lines
    report["enter_lines"] = [ln for ln in g_lines if "hypothesis_G enter" in ln]
    report["leave_lines"] = [ln for ln in g_lines if "hypothesis_G leave" in ln]
    report["hook_lines"] = [ln for ln in g_lines if "hypothesis_G hooked" in ln]

#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""假设 D 验收：arrCatalog 非空 + window 就绪后再调 onReset 无参 ORIG。"""
...
OUT = ROOT / "fixtures" / "_accept_hypothesis_d.json"
...
def resolve_ipa() -> Path:
    env = __import__("os").environ.get("HYPOTHESIS_D_IPA")
    if env:
        p = Path(env)
        if p.is_file():
            return p
    for pat in (
        ...
        if hits:
            return hits[0]
    raise FileNotFoundError("未找到 legado-debug IPA，请设置 HYPOTHESIS_D_IPA 或先下载 CI artifact")
...
def main() -> int:
    ...
    d_lines = [ln for ln in blob.splitlines() if "hypothesis_D" in ln]
    c_lines = [ln for ln in blob.splitlines() if "hypothesis_C" in ln]
    b2_lines = [ln for ln in blob.splitlines() if "hypothesis_B2" in ln]
    report["d_lines"] = d_lines
    report["c_lines"] = c_lines
    report["b2_lines"] = b2_lines[-15:] if b2_lines else []
    report["pre_fire_lines"] = [ln for ln in d_lines if "pre_fire" in ln]
    report["defer_lines"] = [ln for ln in d_lines if "defer_onReset" in ln]
    ...
    report["after_orig_d"] = [ln for ln in d_lines if "after_ORIG pageContainerA=" in ln]
    ...
    if not report["pageContainerA_class"] and report["after_orig_d"]:
        m2 = re.search(r"pageContainerA=(\S+)", report["after_orig_d"][-1])
        if m2:
            report["pageContainerA_class"] = m2.group(1)

    pre_fire = report["pre_fire_lines"][-1] if report["pre_fire_lines"] else ""
    m3 = re.search(r"cat=(\d+)", pre_fire)
    report["catalog_count_at_fire"] = int(m3.group(1)) if m3 else None
    ...
    cat_at_fire = report.get("catalog_count_at_fire")
    report["springboard_or_shelf"] = on_shelf

    if on_shelf:
        report["verdict"] = "FAIL_SPRINGBOARD_OR_SHELF"
    elif fired and orig_ok and container_ok:
        report["verdict"] = "PASS_D"
    elif fired and orig_ok and not container_ok and cat_at_fire is not None and cat_at_fire >= 1:
        report["verdict"] = "STOP_D_CAT_OK_NO_CONTAINER"
    elif fired and orig_ok and not container_ok:
        report["verdict"] = "PARTIAL_D_NO_CONTAINER"
    elif fired and not orig_ok:
        report["verdict"] = "FAIL_D_NO_ORIG_OK"
    elif report["defer_lines"] and not fired:
        report["verdict"] = "FAIL_D_DEFER_ONLY"
    else:

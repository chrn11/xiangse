#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""假设 L 验收：kick raw DR + deliver 门闩 + legado-debug dump。"""
...
OUT = ROOT / "fixtures" / "_accept_hypothesis_l.json"

IPA = ROOT / "dist-ci-l" / "dist" / "StandarReader-legado-bridge-debug.ipa"
...
def evaluate(trace_text: str, marker_text: str, dump_text: str, xiaoyan: dict | None) -> dict:
    ...
    if not has_dr_ok:
        ...
        verdict = "PARTIAL_HANDOFF_M"
        reason = "无崩溃但 pm=0，交 M"
    else:
        verdict = "FAIL"
        reason = "未命中验收条件"
...
def main() -> int:
    ...
    time.sleep(2.5)
    try:
        c.call("open_url", {"url": "legado://debugDump?phase=hypothesis_l"})
    ...
    if report["verdict"] == "PASS":
        return 0
    if report["verdict"] == "PARTIAL_HANDOFF_M":
        return 5
    if report["verdict"] == "FAIL_REVERT_L":
        return 1
    return 1

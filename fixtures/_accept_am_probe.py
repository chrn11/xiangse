#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""AM 验收：post-cb 轻量心跳 + 扩 syslog exit reason + am_icu_caller；KEEP AK/inThread；前台硬断言萧炎。"""
...
OUT = ROOT / "fixtures" / "_accept_am_probe.json"
SYNC = ROOT / "fixtures" / "_accept_am_probe_sync.json"
KILL = ROOT / "fixtures" / "_accept_am_kill_syslog.json"
    ...
    *,
    ...
    if (
        ...
        return {
            ...
            "action": "主线程 QF + 萧炎 + pid 稳；AM 成功可最小修落盘",
            "commit": True,
        }
    if am_hb_n > 0 and (am_icu_n > 0 or exit_reason_n > 0 or not pid_stable):
        return {
            "branch": "am_hb_icu_or_exit",
            "action": "心跳已钉死窗；对照 ICU caller / exit reason；无最小修则只交取证",
            "commit": False,
        }
    if has_ak_skip and scene_syslog_n == 0 and not pid_stable:
        return {
            "branch": "ak_keep_still_kill",
            "action": "AK KEEP 仍成立但 pid 变；AM 证据不足则加深",
            "commit": False,
        }
    if not pid_stable:
        return {
            "branch": "pid_changed",
            "action": "pid 不稳；对照 syslog exit reason / am_hb 末拍",
            "commit": False,
        }
    return {
        "branch": "am_incomplete",
        "action": "AM 证据不足",
        "commit": False,
    }
...
def main() -> int:
    ...
    report: dict = {
        ...
        "ipa": str(ipa),
        "hypothesis": "AM",
        "role": "AM-post-cb-hb-icu-caller-exit-reason",
    ...
    try:
        if reader_ok:
            c.call("open_url", {"url": "legado://debugDump?phase=am_accept"}, timeout=20)
            time.sleep(1.5)

#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""假设 M 验收：DR encoding 槽位修正 + divisionTextRaw + legado-debug dump。"""
...
OUT = ROOT / "fixtures" / "_accept_hypothesis_m.json"

IPA = ROOT / "dist-ci-m" / "dist" / "StandarReader-legado-bridge-debug.ipa"

LENGTH_CRASH_RE = re.compile(
    r"-\[(?:__)?(?:NSSingleObjectArrayI|NSArrayM|NSArrayI) length\]"
)
...
def has_length_crash(marker_text: str, trace_text: str) -> bool:
    blob = marker_text + "\n" + trace_text
    return bool(LENGTH_CRASH_RE.search(blob))
...
def evaluate(trace_text: str, marker_text: str, dump_text: str, xiaoyan: dict | None) -> dict:
    ...
    length_exc = has_length_crash(marker_text, trace_text)
    has_dr_ok = bool(traces["divisionResponse_OK"])
    has_raw = bool(traces["DR_variant=raw"])
    has_flat_retry = bool(traces["DR_variant=flat_retry"])
    has_enc = bool(traces["DR_enc="])
    ...
    if not has_dr_ok:
        verdict = "FAIL_REVERT_M"
        reason = "无 divisionResponse_OK"
    elif length_exc:
        verdict = "FAIL_REVERT_M"
        reason = "仍有 NSArray length 类崩溃"
    elif render_ok:
        verdict = "PASS"
        reason = "无 length 崩溃且 pageModel/萧炎 上屏"
    elif not render_ok and not length_exc and has_dr_ok:
        verdict = "PARTIAL_HANDOFF_N"
        reason = "无崩溃但 pm=0，交 N"
    else:
        verdict = "FAIL"
        reason = "未命中验收条件"

    dr_variant = "raw"
    if has_flat_retry and traces["divisionResponse_OK"]:
        last_ok_idx = trace_text.rfind("divisionResponse_OK")
        last_flat_idx = trace_text.rfind("DR_variant=flat_retry")
        if last_flat_idx > 0 and last_flat_idx < last_ok_idx:
            dr_variant = "flat_retry"

    return {
        "verdict": verdict,
        "reason": reason,
        "dr_variant_hit": dr_variant,
        "has_dr_ok": has_dr_ok,
        "has_enc": has_enc,
        "has_raw": has_raw,
        "has_flat_retry": has_flat_retry,
        "has_deliver_skip": has_deliver_skip,
        "length_exc": length_exc,
        "dump_counts": counts,
...
def main() -> int:
    ...
    time.sleep(2.5)
    try:
        c.call("open_url", {"url": "legado://debugDump?phase=hypothesis_m"})
    ...
    if report["verdict"] == "PASS":
        return 0
    if report["verdict"] == "PARTIAL_HANDOFF_N":
        return 5
    if report["verdict"] == "FAIL_REVERT_M":
        return 1
    return 1

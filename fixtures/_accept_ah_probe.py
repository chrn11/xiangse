#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""AH 探针验收：撤 inThread，疏通主队列后走 async_main；禁 mid-chain launch；前台硬断言萧炎。"""
...
OUT = ROOT / "fixtures" / "_accept_ah_probe.json"
SYNC = ROOT / "fixtures" / "_accept_ah_probe_sync.json"
...
def decide(tags: list[str], has_qf: bool, has_fatal: bool, reader_ok: bool, pid_stable: bool) -> dict:
    ...
    qf_main = any("qf_enter" in ln and "ah_main=1" in ln for ln in tags)
    if not reader_ok:
        return {
    ...
    if has("ah_qf_off_main_WARN") or (has("qf_enter") and not qf_main):
        return {
            "branch": "qf_still_off_main",
            "action": "QF 仍非主线程；勿再 sync marshal",
            "commit": False,
        }
    ...
    if (has("qf_enter") or has_qf) and (has("ah_main_drain_slot") or has("qf_dispatch_main_pulse")):
        return {
            "branch": "qf_on_main_drain_ok",
            "action": "主线程 QF + 主队列可排空；按萧炎裁定",
            "commit": True,
        }
    if has("ah_main_drain_slot") or has("qf_dispatch_main_pulse"):
        return {
            "branch": "main_drains_no_qf",
            "action": "主队列可排空但无 QF；卡在 QF 前",
            "commit": False,
        }
    return {
        "branch": "main_still_starved",
        "action": "主队列仍不排空；AH 疏通失败",
        "commit": False,
    }
...
def main() -> int:
    ...
    report: dict = {
        ...
        "ipa": str(ipa),
        "hypothesis": "AH",
        "role": "AH-main-queue-drain-async-main-QF",
        "model": "cursor-grok-4.5",
        "mock": MOCK,
        "mcp": MCP,
        "banned": ["bounce", "dontFormat", "mid_chain_launch_app", "dispatch_sync_main_QF"],
        "keep": ["V+W+X+Y+Z", "BQM check/format id return", "forensics_no_steal"],
        "steps": [],
        "frontmost_log": [],
        "pid_log": [],
        "launch_app_calls": [],
    ...
    try:
        if reader_ok:
            c.call("open_url", {"url": "legado://debugDump?phase=ah_accept"}, timeout=20)
            time.sleep(1.5)
    ...
    first_chapter = bool(
        ...
        and any("ah_no_inThread_inject" in ln for ln in ac_all)
        and any("path=async_main" in ln for ln in ac_all)
        and any("qf_enter" in ln and "ah_main=1" in ln for ln in ac_all)
        and pid_stable
    )
    ...
    sync = {
        ...
        "ah_last_runtime": ac_runtime[-1] if ac_runtime else (ac_all[-1] if ac_all else ""),
        "ah_runtime": ac_runtime[-60:],
        ...
        "has_ah_no_inThread": any("ah_no_inThread_inject" in ln for ln in ac_all),
        "has_ah_main_drain": any("ah_main_drain_slot" in ln for ln in ac_all),
        "has_ah_qf_main": any("qf_enter" in ln and "ah_main=1" in ln for ln in ac_all),
        "has_ah_qf_off_main": any("ah_qf_off_main_WARN" in ln for ln in ac_all),

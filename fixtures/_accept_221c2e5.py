#!/usr/bin/env python3
"""Acceptance 221c2e5: install CI IPA + nativeRead + assert 萧炎 + trace gates."""
import base64
import json
import time
...
IPA_LOCAL = Path(__file__).resolve().parents[1] / "dist-ci-221c2e5/dist/StandarReader-legado-bridge.ipa"
OUT_DIR = Path(__file__).resolve().parent


def call(tool, arguments=None, timeout=300):
...
def main():
    ...
    dest = "/var/mobile/Library/Caches/ios-mcp-uploads/StandarReader-legado-bridge-221c2e5.ipa"
    ipa_url = "http://192.168.1.4:8765/StandarReader-legado-bridge.ipa"
    dl = call("download_url", {
        "url": ipa_url,
        "dest_path": dest,
        "overwrite": True,
    }, timeout=600)
    ipa_path = dest
    steps.append(f"download={dl}")
    ...
    img_path = OUT_DIR / "_accept_screenshot_221c2e5.png"
    ...
    if doc:
        ...
        once = call("read_file", {
            "path": f"{doc}/legado_native_open_once.txt",
            "max_bytes": 512,
        })
        once_text = once.get("content", "") if isinstance(once, dict) else str(once)
    ...
    skip_lines = [
        ...
        if any(k in ln for k in [
            ...
            "tryOpen skip", "catalogUI skip", "catalogReapply skip",
            "abort push chapterDone", "SIGNAL sig=",
        ])
    ]
    out = {
        "commit": "221c2e5",
